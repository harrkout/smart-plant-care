...
from .forms.irrigationGraph import irrigationGraph
...
@irrigation.route("/irrigation_graph/<int:crop_id>", methods=['GET','POST'])
def irrigation_graph(crop_id):
    ...
    form = irrigationGraph()

    if request.method == 'GET':
        return render_template("irrigation_graph_form.html", crop=crop, form=form)
    ...
    p = figure(title = _('Irrigation'), x_axis_label = _('Date'), x_axis_type = "datetime", y_axis_label = _('Irrigation Needs (κ.μ./στρ.)'), width = 600, height = 350)
    ...
    hover_tool = HoverTool(
        tooltips = [
        (_('Date'), '@x{%F}'),
        (_('Irrigation Needs'), '@y1{0.0}'),
        (_('Irrigation Dose'), '@y2{0.0}')
        ],
        formatters={
            '@x' : 'datetime'
        },
    ...
    p.line('x','y1', legend_label = _('Irrigation Needs'), color ='red', line_width =3,source = source)
    p.line('x','y2', legend_label = _('Irrigation Dose'), color = 'yellow', line_width =3, source = source)
    p.add_tools(hover_tool)
    script, div = components(p)
    resources = CDN.render() 

    return render_template("irrigation_graph.html", crop=crop, form=form, graph_data=graph_data,resources=resources,script=script,div=div)
